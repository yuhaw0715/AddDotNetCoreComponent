...
- 分支：`main`
- 目前已推送基準：`main` 與 `origin/main` 應於本次 7.5 task commit 同步；實際 commit 以 `git log -1` 驗證。
- OpenSpec change：`build-dotnet-scaffold-studio`
- OpenSpec 進度：42/55；最終狀態一律以 `tasks.md` 與 `openspec instructions apply` 的輸出為準。
- 本次 7.5 已完成驗證，commit/push 依使用者要求在本次交付中執行；後續每個 task 仍須獨立完成驗證後再交付。
...
5. OpenSpec 6.1–6.7 已完成 schema 表單狀態、欄位驗證、三類命令工廠、輸出衝突檢查與可取消產生流程；7.1–7.5 已建立桌面導覽、工作區狀態、功能搜尋、參數編輯與確認骨架，但正式產生流程尚未接入 Avalonia UI。
6. UI 是操作流程 Demo；完整狀態呈現、Finder 動作與資源化/可存取性仍待 7.6–7.7 實作。現有 UI 測試主要驗證 ViewModel，不是完整 headless Avalonia smoke suite。
...
1. 建立即時輸出、階段狀態、取消動作、結果摘要、檔案/Git 差異及 Finder 開啟動作。
2. 接著完成 7.7 的資源化、可存取性與鍵盤 UI 驗證。
3. 每完成一項即執行適用測試、更新 `tasks.md`，不要一次提前勾選整個區段。

## 最近一次完整驗證（2026-09-22）

OpenSpec 7.5 完成後的驗證結果：

- Build：0 警告、0 錯誤。
- Test：117 項通過（Integration 37、UI 14、Unit 66）；另有 6.7 workflow preflight、確認/取消與三條暫存專案端到端案例，以及 7.1 導航/收合/狀態保留、7.2 空/單一/多專案與掃描失敗、7.3 搜尋/平台限制、7.4 schema/驗證/預覽/遮蔽、7.5 分類確認/取消案例通過。
...
下一個實作目標是 OpenSpec 7.6：建立即時輸出、階段狀態、取消動作、結果摘要、檔案/Git 差異及 Finder 開啟動作。完成後只在對應驗證全部通過時勾選 7.6，不要提前開始 7.7。
