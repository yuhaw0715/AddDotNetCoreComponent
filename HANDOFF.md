...
- 分支：`main`
- 目前已推送基準：`main` 與 `origin/main` 應於本次 6.4 task commit 同步；實際 commit 以 `git log -1` 驗證。
- OpenSpec change：`build-dotnet-scaffold-studio`
- OpenSpec 進度：34/55；最終狀態一律以 `tasks.md` 與 `openspec instructions apply` 的輸出為準。
- 本次 6.4 已完成驗證，commit/push 依使用者要求在本次交付中執行；後續每個 task 仍須獨立完成驗證後再交付。
...
5. OpenSpec 6.1–6.4 已完成 schema 表單狀態、欄位驗證、`dotnet new` 與 ASP.NET Core Scaffolding 命令工廠，但尚未接入 Avalonia UI；EF Core 命令工廠與端到端產生流程尚未完成，OpenSpec 6.5–6.7 尚未完成。
...
1. 實作規格內 EF Core 命令工廠與 `database update` 二次確認資料。
2. 依序完成 6.6–6.7 的衝突檢查與端到端流程。
3. 每完成一項即執行適用測試、更新 `tasks.md`，不要一次提前勾選整個區段。

## 最近一次完整驗證（2026-09-22）

OpenSpec 6.4 完成後的驗證結果：

- Build：0 警告、0 錯誤。
- Test：93 項通過（Integration 36、UI 2、Unit 55）；另有 6.4 八種 Scaffolding generator snapshot、驗證失敗與錯誤功能拒絕測試通過。
...
下一個實作目標是 OpenSpec 6.5：建立規格內 EF Core 命令工廠與 `database update` 二次確認資料。完成後只在對應驗證全部通過時勾選 6.5，不要提前開始 6.6。
