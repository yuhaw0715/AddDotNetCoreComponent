...
- OpenSpec 5.5 已完成：`DependencyPlanWorkflow` 以一次性計畫確認 token 執行結構化相依性步驟，逐步重驗證並保留部分成功、失敗與 Git／檔案差異。下一個待辦是 5.6，仍以 `tasks.md` 和最新 `openspec instructions apply` 結果為準。
