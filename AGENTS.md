...
- OpenSpec 5.1 已完成：`IDotNetEnvironmentDiscovery` 透過 `CommandRequest` 與 `ProcessCommandRunner` 探索 .NET SDK、Runtime、工作負載及工作區本機工具；這是唯讀服務，不安裝全域工具。下一個待辦以 `tasks.md` 和最新 `openspec instructions apply` 結果為準，目前交接建議接續 5.2。
