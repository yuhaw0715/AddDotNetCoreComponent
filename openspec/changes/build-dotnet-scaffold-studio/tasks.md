...
- [x] 5.5 建立 `DependencyPlan` 及計畫—確認—執行—重驗證流程，並以拒絕確認、部分成功與重驗證失敗案例測試狀態與檔案差異。
- [x] 5.6 實作建立工具資訊清單及專案本機工具安裝，不使用全域安裝，並在隔離整合測試中驗證 `.config/dotnet-tools.json` 與工具可執行。
- [x] 5.7 實作只針對已選 `.csproj` 的 NuGet 套件安裝/restore，並以暫存專案驗證套件版本、錯誤階段與失敗時不自動回復。
- [x] 5.8 實作缺少 SDK/工作負載的說明與獨立確認流程，並以假的安裝 adapter 驗證未確認時不啟動任何網路或修改操作。

## 6. 參數、命令與產生流程

- [x] 6.1 實作 schema 驅動表單狀態，支援布林、列舉、路徑、清單、一般文字及機密型別，並以單元測試驗證常用/進階切換不遺失值。
- [x] 6.2 實作必填、名稱、路徑、條件式、相依及互斥驗證，並以 Controller、Razor Page、Blazor、EF Core 代表案例驗證欄位級中文錯誤。
- [x] 6.3 實作 `dotnet new` 專案、項目與設定檔命令工廠，並以 46 個官方範本的參數化測試驗證短名稱、工作目錄及目標輸出。
...
- [x] 7.1 建立 Avalonia 主視窗、可折疊左側導覽及內容區，涵蓋首頁/工作區、專案範本、元件與檔案、Scaffolding、EF Core、自訂範本、歷程與設定，並以 UI 測試驗證導航。
- [x] 7.2 建立工作區選擇、掃描結果及目標專案選擇畫面，並以空工作區、單一與多專案 UI 測試驗證狀態。
- [x] 7.3 建立功能卡片/清單及搜尋篩選，呈現可用性、相依性、風險與平台限制，並以 macOS UI 測試驗證 WPF/WinForms 可見但停用。
- [x] 7.4 建立 schema 驅動參數編輯器、進階選項與唯讀命令預覽，並以 UI 測試驗證即時更新、驗證訊息與機密遮蔽。
- [x] 7.5 建立一般執行、覆寫、工具安裝及 `database update` 對應確認對話框，並以 UI 測試驗證取消不會呼叫執行服務。
- [x] 7.6 建立即時輸出、階段狀態、取消動作、結果摘要、檔案/Git 差異及 Finder 開啟動作，並以成功、失敗、取消 fixture 驗證畫面。
- [x] 7.7 將所有使用者可見固定文字移至繁體中文資源並加上可存取名稱、焦點順序與非顏色狀態提示，以資源掃描及鍵盤 UI 測試驗證。
