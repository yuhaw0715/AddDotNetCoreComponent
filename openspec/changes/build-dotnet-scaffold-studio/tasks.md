...
- [ ] 5.1 實作 `dotnet --info`、SDK/Runtime、工作負載與工具清單偵測，並以缺少 SDK、只有 Runtime、多 SDK 及命令失敗 fixture 驗證狀態。
- [ ] 5.2 實作固定 CLI UI 語言的 `dotnet new list` 探索與解析，並以多版本/不同欄寬輸出 fixture 驗證官方與自訂範本辨識。
- [ ] 5.3 實作自訂範本 `--help` 解析及安全降級模式，並以可解析、部分解析與無法解析 fixture 驗證不會產生 shell 命令入口。
